...
class Utilisateur{
 ...
 $this->db = $db;
 $this->insert = $db->prepare("insert into utilisateur(email, mdp, nom, prenom, dateCrea, idRole) values(:email, :mdp, :nom, :prenom, :dateCrea, :role)");
 $this->connect = $db->prepare("select email, idRole, mdp from utilisateur where email=:email");
 $this->select = $db->prepare("select  id, email, idRole, nom, prenom, dateCrea, r.libelle as libellerole from utilisateur u, role r where u.idRole = r.id order by nom");
 ...
 public function insert($inputEmail, $inputPassword, $nom, $prenom, $dateCrea, $role){
 $r = true;
 $this->insert->execute(array(':email'=>$inputEmail, ':mdp'=>$inputPassword, ':nom'=>$nom, ':prenom'=>$prenom, ':dateCrea'=>$dateCrea, ':role'=>$role));
